    ...
    open(component: Component, id?: number | any): NgbModalRef {
        ...
        if (id) {
            this.applicationService.find(id).subscribe((application) => {
                if (application.seedingDate) {
                    application.seedingDate = {
                        year: application.seedingDate.getFullYear(),
                        month: application.seedingDate.getMonth() + 1,
                        day: application.seedingDate.getDate()
                    };
                }
